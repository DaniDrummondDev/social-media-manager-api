...
final class PublishNowUseCase
{
    ...
    public function execute(PublishNowInput $input): SchedulePostOutput
    {
        ...
        if ($content->status !== ContentStatus::Ready) {
            throw new PublishingNotAllowedException(
                "Content must be in 'ready' status to publish. Current status: '{$content->status->value}'.",
            );
        }
