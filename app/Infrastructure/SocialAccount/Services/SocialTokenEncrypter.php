...
final class SocialTokenEncrypter
{
    ...
    {
        $key = config('social-media.encryption.key');

        if (empty($key)) {
            throw new RuntimeException('SOCIAL_TOKEN_ENCRYPTION_KEY is not configured.');
        }

        $this->key = base64_decode($key);

        if (strlen($this->key) !== 32) {
            throw new RuntimeException('SOCIAL_TOKEN_ENCRYPTION_KEY must be 32 bytes (base64 encoded).');
        }
    }
